...
| `/범프알림` | 범프 알림 멘션을 받도록 켜기/끄기(본인) | 누구나 |
| `/기억추가 <내용>` | 서버 전체가 참고할 지식 저장(관리자) | 서버 관리 |
| `/기억목록` · `/기억삭제 <번호>` | 서버 지식 보기 / 삭제 | 서버 관리 |
| `/내기억추가 <내용>` · `하루야 … 기억해` | 내 개인 기억 저장(내 대화에서만 참고) | 누구나 |
...
> AI 기억은 **서버 지식(관리자 `/기억추가`, 전체 공유)** 과 **개인 기억(누구나 `하루야 … 기억해`/`/내기억추가`, 본인 대화에만)** 으로 분리됩니다. 모델 학습이 아니라 기억을 프롬프트에 주입하는 방식입니다.
...
- `/범프알림` 으로 **구독한 유저는 알림 때 @멘션(핑)** 으로 받습니다(직접 켜고/끔).
...
│   ├── ai_chat.py      # "하루야 …" 대화/번역(Gemini↔Groq) + 임의 답장 + 서버지식/개인기억/맥락
